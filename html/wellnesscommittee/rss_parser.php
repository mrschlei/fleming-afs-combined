...
define ("RSSUNAVAILABLEERROR", "Content not available. Please try back shortly.If you have an immediate need, please <a href='maito:wellnessblog'>contact the Webmaster</a>");
define ("SINGLEDETAILERROR", "<div style='font-size:10px;'>There are no comments for this post yet. All comments are moderated, so if you've submitted a comment recently, it may be a few hours before it appears in this space.</div>");
...
function get_item($xml_param, $item_param) {
		...
		try{
			...
			//The comment feed doesn't use the <link> tag, so if it the feed given doesn't have a category tag,
			//just do the comment formatting, otherwise, use the full post formatting
			$category = $item[0]->category;
			
			if (!$category) {
			echo "<div class=\"comments\">".$item[0]->description."<br><div id='postdate' align='right'>".$item[0]->pubDate."</div></div>";
			}
			...
			echo "<span id='postdate'><!--Posted by <a href='http://directory.umich.edu/ldapweb-bin/url?ldap:///uid=".$author.",ou=People,dc=umich,dc=edu'>" . $author. "</a>-->" . $formatted_pubdate. "<br>Category: ".$cattext."</span>";
				}//end comment/post checking else
