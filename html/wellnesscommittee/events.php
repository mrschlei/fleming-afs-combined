...
#eventsTable th {border:1px solid #6699cc;}
</style>
...
$eventcompare = mktime(0, 0, 0, $month, $day, $year);
//date for the event
$date = date("jS", $eventcompare);
//date right now
$nowcompare = mktime(0, 0, 0, $currentmonth, $currentday, $thisyear);


//if we put $day >= $currentday, then an event on April 29th won't show up when the current day is March 30th
if ($eventcompare >= $nowcompare) { 
...
if ($row['location']) {
$event .= "<p>".$row['location']."</p>";
}

if ($row['pdf']) {
	$event .= "<p><a href='".$row['pdf']."' target='_blank'><img src='/images/pdficon.gif' alt='PDF' border='0' /></a> <a href='".$row['pdf']."' target='_blank'>Flyer</a></p></td></tr>";
}//end if
